"""
what_if_brazil_norway.py — What-If Analysis: Brazil vs Norway R16 Outcome Scenarios.

Forces each possible result of Brazil vs Norway (R16-3) and simulates the
...
def main() -> int:
    ...
    # Current Brazil vs Norway prediction
    bra_row = predictions[(predictions["home_team"] == "Brazil")].iloc[0]
    print(f"\n{'#' * 72}")
    print(f"  #  WHAT-IF: BRAZIL vs NORWAY (R16-3)")
    print(f"  #  Current prediction:")
    print(f"  #    Brazil wins:  {bra_row['home_win_prob']:.1%}")
    print(f"  #    Draw:         {bra_row['draw_prob']:.1%}")
    print(f"  #    Norway wins:  {bra_row['away_win_prob']:.1%}")
    print(f"  #    Confidence:   {bra_row['home_win_prob']:.0%}")
    print(f"{'#' * 72}")

    # Path analysis
    print(f"\n  Brazil's projected bracket path:")
    print(f"  R16:  Norway    → Brazil")
    print(f"  QF:   England/Mexico → Brazil (? SF-1)")
    print(f"  SF:   Morocco/France   → Brazil (? Final)")
    print(f"  Final: Portugal/Spain → Brazil (Champion?)")

    print(f"\n  Norway's projected path if they win:")
    print(f"  R16:  Brazil    → Norway")
    print(f"  QF:   England/Mexico → Norway (? SF-1)")
    print(f"  SF:   Morocco/France   → Norway (? Final)")
    print(f"  Final: Portugal/Spain → Norway (??)")

    # ── Scenarios ──
    scenarios = {
        "Default": None,
        "Brazil wins": {"R16-3": "Brazil"},
        "Norway wins": {"R16-3": "Norway"},
    }
    ...
    bra_mc = mc_results["Brazil wins"]
    nor_mc = mc_results["Norway wins"]

    print(f"{'─' * 72}")
    print(f"  KEY TAKEAWAYS")
    print(f"{'─' * 72}")

    print(f"\n  Default champion:         {det_results['Default']['champion']}")
    print(f"  If Brazil wins R16:      {det_results['Brazil wins']['champion']}")
    print(f"  If Norway wins R16:      {det_results['Norway wins']['champion']}")
    print()

    # Probability swings
    print(f"  {'Team':<18} {'Default':>8} {'Brazil wins':>14} {'Norway wins':>15} {'Δ Bra':>7} {'Δ Nor':>7}")
    print(f"  {'─' * 18} {'─' * 8} {'─' * 14} {'─' * 15} {'─' * 7} {'─' * 7}")
    for team in list(def_mc.keys())[:12]:
        d = def_mc.get(team, 0)
        b = bra_mc.get(team, 0)
        n = nor_mc.get(team, 0)
        db = b - d
        dn = n - d
        print(f"  {team:<18} {d:>5.1f}%   {b:>5.1f}% ({db:+>+.1f})  {n:>5.1f}% ({dn:+>+.1f})  {db:>+5.1f}%  {dn:>+5.1f}%")
    print()

    # Biggest beneficiaries
    print(f"  Teams that benefit most if Norway (underdog) wins R16:")
    nor_boost = [(team, nor_mc.get(team, 0) - def_mc.get(team, 0))
                 for team in def_mc if (nor_mc.get(team, 0) - def_mc.get(team, 0)) > 0.3]
    nor_boost.sort(key=lambda x: -x[1])
    for team, delta in nor_boost[:6]:
        print(f"    {team}: {delta:+.1f}pp")

    print(f"\n  Teams hurt most if Norway wins (Brazil eliminated):")
    nor_hurt = [(team, def_mc.get(team, 0) - nor_mc.get(team, 0))
                for team in def_mc if (def_mc.get(team, 0) - nor_mc.get(team, 0)) > 0.3]
    nor_hurt.sort(key=lambda x: -x[1])
    for team, delta in nor_hurt[:6]:
        print(f"    {team}: -{delta:.1f}pp")
