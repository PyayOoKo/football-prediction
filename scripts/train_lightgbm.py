#!/usr/bin/env python3
"""
Train and validate LightGBM classifier using the full feature engineering pipeline.

Usage:
    python scripts/train_lightgbm.py
    python scripts/train_lightgbm.py --quiet
"""
...
def main() -> int:
    ...
    if args.tune:
        ...
        result = tune_hyperparameters(
            ...
            model_type="lightgbm", n_folds=args.tune_folds,
    ...
    step += 1
    print(f"\n[{step}/{total_steps}] Training LightGBM...")
    ...
    # Save model
    model_path = save_model(model, "lightgbm_model")
    print(f"  Model saved: {model_path}")

    # Save validation report
    report_dir = PROJECT_ROOT / "reports"
    report_dir.mkdir(parents=True, exist_ok=True)
    report_path = report_dir / f"lightgbm_validation_{timestamp}.json"
    report_data = {
        "timestamp": timestamp,
        "model": "lightgbm",
        "status": "PASS",
