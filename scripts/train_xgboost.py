#!/usr/bin/env python3
"""
Train and validate XGBoost classifier using the full feature engineering pipeline.

Usage:
    python scripts/train_xgboost.py
    python scripts/train_xgboost.py --quiet
"""
...
def main() -> int:
    ...
    # Add target column (map H/D/A -> 2/1/0)
    ...
    # Keep raw test set for BTTS/O/U computation
    ...
    if args.tune:
        ...
        result = tune_hyperparameters(
            ...
            model_type="xgboost", n_folds=args.tune_folds,
            n_iter=args.tune_iter, verbose=not args.quiet,
        )
        print(f"  Best params: {result['best_params']}")
        # Apply best params to config (all known params are now in TrainConfig)
    ...
    # Derive BTTS/O/U probabilities from outcome probabilities
    # using conditional rates from training data
    def _conditional_rates(df_train_cond):
        hw = df_train_cond[df_train_cond["result"] == "H"]
        d = df_train_cond[df_train_cond["result"] == "D"]
        aw = df_train_cond[df_train_cond["result"] == "A"]
    ...
    # probs: [away_win, draw, home_win] (from target mapping A=0, D=1, H=2)
    ...
    metrics["split_ratios"] = "0.6/0.2/0.2"

    print(f"  BTTS accuracy: {btts_acc:.4f}  |  O/U accuracy: {ou_acc:.4f}")

    # Save model
    model_path = save_model(model, "xgboost_model")
    print(f"  Model saved: {model_path}")

    # Save validation report
    report_dir = PROJECT_ROOT / "reports"
    report_dir.mkdir(parents=True, exist_ok=True)
    report_path = report_dir / f"xgboost_validation_{timestamp}.json"
    report_data = {
        "timestamp": timestamp,
        "model": "xgboost",
        "status": "PASS",
        "metrics": {k: v for k, v in metrics.items() if k != "classification_report" and k != "plots"},
        "tuned": args.tune,
        "tune_folds": args.tune_folds if args.tune else None,
    }
    # Remove unserializable keys (plots paths, classification_report text)
    report_data["metrics"].pop("plots", None)
