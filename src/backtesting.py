...
class BacktestEngine:
    ...
    def print_report(self) -> None:
        ...
        if m.total_bets == 0:
            ...
            print(f"\n  Final bankroll: £{m.final_bankroll:.2f}")
            print("=" * 90)
            return
        ...
        print(f"  {'Total staked':<30} £{m.total_staked:>12.2f}   {'Sum of all stakes':<35}")
        print(f"  {'Total profit / loss':<30} £{m.total_profit:>+11.2f}   {'+ = profit, - = loss':<35}")

        # ROI
        roi_color = "🟢" if m.roi_pct > 0 else "🔴" if m.roi_pct < 0 else "⚪"
        print(f"  {'ROI (Return on Investment)':<30} {roi_color} {m.roi_pct:>+9.2f}%   {'Total return on initial bankroll':<35}")

        # Yield
        yield_color = "🟢" if m.yield_pct > 0 else "🔴" if m.yield_pct < 0 else "⚪"
        print(f"  {'Yield (profit / staked)':<30} {yield_color} {m.yield_pct:>+9.2f}%   {'Return per unit staked':<35}")

        # Final bankroll
        bankroll_change = m.final_bankroll - m.initial_bankroll
        change_str = f"+£{bankroll_change:.2f}" if bankroll_change >= 0 else f"-£{abs(bankroll_change):.2f}"
        print(f"  {'Final bankroll':<30} £{m.final_bankroll:>11.2f}   ({change_str} from £{m.initial_bankroll:.0f})")

        # Drawdown
        print(f"  {'Max drawdown':<30} {m.max_drawdown_pct:>12.2f}%   (£{m.max_drawdown_amount:.2f} peak-to-trough)")
    ...
    def _plot_bankroll_curve(
        ...
        ax.axhline(self.initial_bankroll, color="#555555", linestyle="--",
                   linewidth=0.8, alpha=0.6, label=f"Starting bankroll (£{self.initial_bankroll:.0f})")
        ...
        ax.text(0.98, 0.05,
                f"Start: £{self.initial_bankroll:.2f}\n"
                f"Final: £{end_val:.2f}\n"
                f"ROI: {m.roi_pct:+.2f}%\n"
                f"Bets: {m.total_bets}",
    ...
    def _plot_cumulative_profit(
        ...
        ax.text(0.98, 0.05,
                f"Total profit: £{final_profit:+.2f}\n"
    ...
    def _plot_bet_outcomes(
        ...
        ax.set_xlabel("Bet Number")
        ax.set_ylabel("Profit/Loss per Bet (£)")
        ax.set_title("Backtest — Individual Bet Outcomes", fontweight="bold", fontsize=14)
        ax.legend(fontsize=9, loc="upper left")

        # Size legend note
        ax.text(0.98, 0.05,
                "Marker size ∝ stake amount\n"
                f"Bets: {m.total_bets}  |  "
                f"Avg stake: £{m.total_staked / m.total_bets:.1f}",
                transform=ax.transAxes, fontsize=9, verticalalignment="bottom",
                horizontalalignment="right",
                bbox=dict(boxstyle="round,pad=0.5", facecolor="white", alpha=0.9))
...
1.  ROI (RETURN ON INVESTMENT)
    ───────────────────────────
    Formula:    ROI = (final_bankroll - initial_bankroll) / initial_bankroll × 100

    What it means:
    The total percentage return on your starting bankroll.
    ROI = +15% means you turned £1,000 into £1,150.
    This is the headline metric — positive is good, negative is bad.

2.  YIELD
    ──────
    Formula:    Yield = total_profit / total_staked × 100

    What it means:
    Return per unit staked.  If you bet £5,000 total and made £250 profit,
...
4.  WIN RATE
    ─────────
    Formula:    Win Rate = winning_bets / total_bets × 100
...
5.  MAXIMUM DRAWDOWN
    ...
    bankroll grew to £1,200 then fell to £900, that's a 25% drawdown.
