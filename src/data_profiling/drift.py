...
class DriftReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "metrics": [
                {
                    "name": m.name,
                    "previous": str(m.previous_value),
                    "current": str(m.current_value),
    ...
    def summary_text(self) -> str:
        ...
        for m in self.metrics:
            ...
            lines.append(
                f"   {icon} {m.name}: {m.previous_value} → {m.current_value} ({m.delta:+.1%})"
            )
        return "\n".join(lines)
...
class DataDriftDetector:
    ...
    def _check_row_count(
        self, current: ProfilingReport, previous: ProfilingReport, report: DriftReport
    ) -> None:
        ...
        severity = (
            "info"
            if abs(delta) < self.row_count_threshold
            else "warning"
            if abs(delta) < self.row_count_threshold * 3
            else "critical"
        )
    ...
    def _check_null_drift(
        ...
        for col in set(
            list(prev_mv.get("columns", {}).keys())
            + list(curr_mv.get("columns", {}).keys())
        ):
            ...
            if diff > self.null_pct_threshold:
                severity = (
                    "warning" if diff < self.null_pct_threshold * 2 else "critical"
                )
                report.metrics.append(
                    DriftMetric(
                        name=f"Null % - {col}",
    ...
    def _check_result_distribution_drift(
        self, current: ProfilingReport, previous: ProfilingReport, report: DriftReport
    ) -> None:
    ...
    def _check_home_advantage_drift(
        self, current: ProfilingReport, previous: ProfilingReport, report: DriftReport
    ) -> None:
