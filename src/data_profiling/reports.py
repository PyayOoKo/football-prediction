...
<div class="container">
  ...
  <div class="footer">
    ...
    def _summary_card(
        self, label: str, value: Any, unit: str = "", badge: str = "ok"
    ) -> str:
...
<div class="chart-container">{fig.to_html(full_html=False, include_plotlyjs=False, default_width="100%", default_height="100%")}</div>"""
    ...
    def _render_column_summary(self) -> str:
        ...
        for _, r in display.iterrows():
            badge = (
                "ok" if r["null_pct"] == 0 else "warn" if r["null_pct"] < 20 else "err"
            )
            ...
            if val > 0:
                fig.add_trace(
                    go.Bar(
                        ...
                        text=[
                            f"{val}<br>({data.get('percentages', {}).get(k, 0):.1f}%)"
                        ],
                        textposition="inside",
                    )
                )
    ...
    def _render_home_advantage(self) -> str:
        ...
        fig = go.Figure(
            go.Bar(
                ...
                text=[
                    f"{data['home_win_pct']:.1f}%",
                    f"{data['draw_pct']:.1f}%",
                    f"{data['away_win_pct']:.1f}%",
                ],
                textposition="outside",
            )
        )
...
<div class="chart-container">{fig.to_html(full_html=False, include_plotlyjs=False, default_width="100%", default_height="100%")}</div>"""
    ...
    def _render_outliers(self) -> str:
        ...
        for col, info in sorted(
            cols.items(), key=lambda x: x[1]["n_outliers"], reverse=True
        )[:20]:
...
<table><thead><tr><th>Column</th><th>Outliers</th><th>%</th><th>Mean</th><th>Min Outlier</th><th>Max Outlier</th></tr></thead><tbody>{rows_html}</tbody></table>"""
    ...
    def _render_schema(self) -> str:
        ...
        if not missing:
            ...
            parts.append(
                f'<p style="color:#ef9a9a">⚠ {len(missing)} expected columns missing: {", ".join(missing)}</p>'
            )

        unexpected = data.get("unexpected_columns", [])
        if unexpected:
            parts.append(
                f'<p style="color:#8b8fa3">ℹ {data.get("n_unexpected", 0)} unexpected columns (e.g. {", ".join(unexpected[:8])})</p>'
            )

        return (
            "<br>".join(parts)
            + f"""<table><thead><tr><th>Expected</th><th>Status</th></tr></thead><tbody>
