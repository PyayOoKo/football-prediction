...
def run_eda(
    ...
    logger.info(
        "EDA complete — %d charts saved to %s", len(report["charts"]), _FIGURE_DIR
    )
    return report
...
def _chart_win_distribution(df: pd.DataFrame) -> tuple[plt.Figure, str]:  # type: ignore[name-defined]
    ...
    **Why this matters:** Football is famously low-scoring, which makes draws
    ...
    ax.set_title(
        "Win Distribution — Home / Draw / Away", fontweight="bold", fontsize=14
    )
...
def _chart_home_advantage(df: pd.DataFrame) -> tuple[plt.Figure, str]:  # type: ignore[name-defined]
    ...
    **Why this matters:** Home advantage is the single most reliable finding
    ...
    bars_win = ax.bar(
        x - width, win_pcts, width, label="Win", color="#2ecc71", edgecolor="white"
    )
    bars_draw = ax.bar(
        x, draw_pcts, width, label="Draw", color="#f1c40f", edgecolor="white"
    )
    bars_loss = ax.bar(
        x + width, loss_pcts, width, label="Loss", color="#e74c3c", edgecolor="white"
    )
    ...
    ax.set_title(
        "Home Advantage — Win/Draw/Loss by Venue", fontweight="bold", fontsize=14
    )
    ax.legend(fontsize=10)
...
def _chart_team_statistics(df: pd.DataFrame) -> tuple[plt.Figure, str]:  # type: ignore[name-defined]
    ...
    **Why this matters:** Team-level aggregates reveal which teams are
    ...
    goals_scored = home_scored.add(away_scored, fill_value=0).sort_values(
        ascending=False
    )

    home_conceded = df.groupby("home_team")["away_goals"].sum()
    away_conceded = df.groupby("away_team")["home_goals"].sum()
    goals_conceded = home_conceded.add(away_conceded, fill_value=0).sort_values(
        ascending=False
    )
...
def _chart_correlation_matrix(df: pd.DataFrame) -> tuple[plt.Figure, str]:  # type: ignore[name-defined]
    ...
    **What to look for:**
    ...
    fe_cols = [
        ...
        if c.startswith(("h_", "a_", "h2h_")) and c in numeric.columns
    ]
    ...
    fig, ax = plt.subplots(
        figsize=(max(10, len(selected) * 0.6), max(8, len(selected) * 0.55))
    )
...
def _chart_missing_values(df: pd.DataFrame) -> tuple[plt.Figure, str]:  # type: ignore[name-defined]
    ...
    **Interpreting the heatmap:**
    ...
    if len(missing_df) > 0:
        ...
        colors = [
            "#e74c3c" if p > 50 else "#f39c12" if p > 20 else "#3498db"
        ...
        for bar, cnt, pct in zip(
            bars, top_missing["Count"], top_missing["Percent"], strict=False
        ):
            ax1.text(
                bar.get_width() + 5,
                bar.get_y() + bar.get_height() / 2,
