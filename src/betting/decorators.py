"""
Decorators — logging, timing, and retry for betting modules.

Composable decorators that wrap betting pipeline stages with
cross-cutting concerns without modifying the module logic.
"""
...
# ═══════════════════════════════════════════════════════════
#  Timing — measure and log execution duration
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
#  Logged — log entry/exit with parameters
# ═══════════════════════════════════════════════════════════


def logged(level: int = logging.DEBUG) -> Callable[[F], F]:
    """Decorator that logs method entry and exit with parameters.
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            if logger.isEnabledFor(level):
                ...
                arg_preview = ", ".join(
                    str(a)[:60] for a in args[1:]
                )
                kwarg_preview = ", ".join(
                    f"{k}={str(v)[:60]}" for k, v in kwargs.items()
                )
                logger.log(
                    level, "%s.%s(%s%s)",
