...
def collect_weather(
    ...
    if len(df) < len(matches_df):
        placeholders = _build_placeholder_df(len(matches_df) - len(df), config=cfg)
...
def _parse_owm_response(
    ...
    record = {
        ...
        "feels_like_celsius": current.get(
            "feels_like", _global_config.weather.default_temp
        ),
...
def _geocode_teams(team_names: list[str]) -> dict[str, tuple[float, float]]:
    ...
    for name in team_names:
        try:
            url = f"{OWM_BASE}/geo/1.0/direct?q={name}&limit=1&appid={api_key}"
